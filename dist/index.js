...
var TECH_WORDS = [{
  ...
  callback: function callback() {
    return window.open('https://reactjs.org/', '_blank');
  }
}, {
  label: 'React Native',
  callback: function callback() {
    return window.open('https://reactnative.dev/', '_blank');
  }
}, {
  label: 'NodeJS',
  callback: function callback() {
    return window.open('https://nodejs.org/en/', '_blank');
  }
}, {
  label: 'AWS',
  callback: function callback() {
    return window.open('https://aws.amazon.com/', '_blank');
  }
}, {
  label: 'Styled Components',
  callback: function callback() {
    return window.open('https://styled-components.com/', '_blank');
  }
}, {
  label: 'i18n',
  callback: function callback() {
    return window.open('https://www.i18next.com/', '_blank');
  }
}, {
  label: 'Python',
  callback: function callback() {
    return window.open('https://www.python.org/', '_blank');
  }
}, {
  label: 'Jest',
  callback: function callback() {
    return window.open('https://jestjs.io/', '_blank');
  }
}, {
  label: 'RTL',
  callback: function callback() {
    return window.open('https://testing-library.com/', '_blank');
  }
}, {
  label: 'BDD/ATDD',
  callback: function callback() {
    return window.open('https://en.wikipedia.org/wiki/Behavior-driven_development', '_blank');
  }
}, {
  label: 'Cucumber',
  callback: function callback() {
    return window.open('https://cucumber.io/', '_blank');
  }
}, {
  label: 'Storybook',
  callback: function callback() {
    return window.open('https://storybook.js.org/', '_blank');
  }
}, {
  label: 'JSdoc',
  callback: function callback() {
    return window.open('https://jsdoc.app/', '_blank');
  }
}, {
  label: 'GraphQL',
  callback: function callback() {
    return window.open('https://graphql.org/', '_blank');
  }
}, {
  label: 'Teaching',
  callback: function callback() {
    return window.open('https://en.wikipedia.org/wiki/Teacher', '_blank');
  }
}, {
  label: 'TypeScript',
  callback: function callback() {
    return window.open('https://www.typescriptlang.org/', '_blank');
  }
}, {
  label: 'Docker',
  callback: function callback() {
    return window.open('https://www.docker.com/', '_blank');
  }
}];
...
var Container = newStyled.div(_templateObject || (_templateObject = _taggedTemplateLiteralLoose(["\n  position: relative;\n  align-items: center;\n  justify-content: center;\n"])));
var FloatingText = newStyled.p(_templateObject2 || (_templateObject2 = _taggedTemplateLiteralLoose(["\n  font-size: 1rem;\n  color: #08fdd8;\n  margin: 10px;\n"])));
var FloatingContainer = newStyled.div(_templateObject3 || (_templateObject3 = _taggedTemplateLiteralLoose(["\n  position: absolute;\n  top: 50%;\n  left: 50%;\n  transform: translate3d(-50%, -50%, 0) scale(1);\n  cursor: pointer;\n  &:hover {\n    background-color: rgba(255, 255, 255, 0.1);\n    border: 1px solid #fff;\n    -webkit-border-radius: 5px;\n    -moz-border-radius: 5px;\n    border-radius: 5px;\n    opacity: 1 !important;\n    z-index: 1 !important;\n  }\n"])));

var TagCloud = function TagCloud(_ref) {
  var _ref$words = _ref.words,
      words = _ref$words === void 0 ? TECH_WORDS : _ref$words,
      className = _ref.className;

  var _useState = React.useState(function () {
    var delta = window.innerWidth - 375;
    ...
    return {
      ...
      prevConfig = _useState[0],
      setPrevConfig = _useState[1];
  ...
  var _useState3 = React.useState(function () {
    ...
    words.forEach(function (word, index) {
      var phi = Math.acos(-1 + (2 * index + 1) / words.length);
      var theta = Math.sqrt((words.length + 1) * Math.PI) * phi;
      newArray.push({
  ...
  React.useEffect(function () {
    var onResizeConfig = function onResizeConfig() {
      var delta = window.innerWidth - 375;
  ...
  var _useState4 = React.useState(config.initSpeed * Math.sin(config.direction * (Math.PI / 180))),
      mouseX0 = _useState4[0];

  var _useState5 = React.useState(-config.initSpeed * Math.cos(config.direction * (Math.PI / 180))),
      mouseY0 = _useState5[0];

  var _useState6 = React.useState(config.initSpeed * Math.cos(config.direction * (Math.PI / 180))),
      mouseX = _useState6[0],
      setMouseX = _useState6[1];

  var _useState7 = React.useState(-config.initSpeed * Math.cos(config.direction * (Math.PI / 180))),
      mouseY = _useState7[0],
      setMouseY = _useState7[1];

  var resetMousePos = function resetMousePos(event) {
    var containerPos = document.getElementById('container').getBoundingClientRect();
    setMouseX((event.clientX - (containerPos.left + containerPos.width / 2)) / 5);
    setMouseY((event.clientY - (containerPos.top + containerPos.height / 2)) / 5);
  };

  React.useEffect(function () {
    var loop = setInterval(function () {
      var mX = Math.abs(mouseX - mouseX0) < 1 ? mouseX0 : (mouseX + mouseX0) / 2;
      var mY = Math.abs(mouseY - mouseY0) < 1 ? mouseY0 : (mouseY + mouseY0) / 2;
      var a = -(Math.min(Math.max(-mY, -config.size), config.size) / config.radius) * config.maxSpeed;
      var b = Math.min(Math.max(-mX, -config.size), config.size) / config.radius * config.maxSpeed;
      var l = Math.PI / 180;
      var sc = [Math.sin(a * l), Math.cos(a * l), Math.sin(b * l), Math.cos(b * l)];
      var newArray = [];
      items.forEach(function (item, index) {
        var currentRef = refArray.current[index];
        ...
        var per = 2 * config.depth / (2 * config.depth + rz2);
        newArray.push({
          x: rx2,
          y: ry2,
          z: rz2
        });
        var scale = per.toFixed(3);
        var alpha = per * per - 0.25;
        alpha = (alpha > 1 ? 1 : alpha).toFixed(3);
        var left = (rx2 - currentRef.offsetWidth / 2).toFixed(2);
        var top = (ry2 - currentRef.offsetHeight / 2).toFixed(2);
  ...
  }, [config, config.depth, config.maxSpeed, config.radius, config.size, items, mouseX, mouseX0, mouseY, mouseY0]);
  return /*#__PURE__*/React__default.createElement(Container, {
    className: className,
    onMouseMove: resetMousePos,
    id: "container"
  }, words.map(function (word, index) {
    return /*#__PURE__*/React__default.createElement(FloatingContainer, {
      id: index,
      key: index,
      ref: function ref(el) {
    ...
    }, /*#__PURE__*/React__default.createElement(FloatingText, null, word.label));
  }));
};
