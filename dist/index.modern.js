...
import React__default, { forwardRef, useContext, createContext, createElement, Fragment as Fragment$1, useState, useRef, useEffect } from 'react';
...
const TECH_WORDS = [{
  ...
  label: 'BDD/ATDD',
  callback: () => window.open('https://en.wikipedia.org/wiki/Behavior-driven_development', '_blank')
}, {
  label: 'Cucumber',
  ...
  label: 'Teaching',
  callback: () => window.open('https://en.wikipedia.org/wiki/Teacher', '_blank')
}, {
  label: 'TypeScript',
...
let _$1 = t => t,
    ...
    _t2,
    _t3;
const Container = newStyled.div(_t || (_t = _$1`
  position: relative;
  align-items: center;
  justify-content: center;
`));
const FloatingText = newStyled.p(_t2 || (_t2 = _$1`
  font-size: 1rem;
  color: #08fdd8;
  margin: 10px;
`));
const FloatingContainer = newStyled.div(_t3 || (_t3 = _$1`
  position: absolute;
  top: 50%;
  left: 50%;
...
const TagCloud = ({
  ...
  const [items, setItems] = useState(() => {
    ...
    _words.forEach((word, index) => {
      const phi = Math.acos(-1 + (2 * index + 1) / _words.length);
      const theta = Math.sqrt((_words.length + 1) * Math.PI) * phi;
      newArray.push({
  ...
  const [mouseX0] = useState(config.initSpeed * Math.sin(config.direction * (Math.PI / 180)));
  const [mouseY0] = useState(-config.initSpeed * Math.cos(config.direction * (Math.PI / 180)));
  const [mouseX, setMouseX] = useState(config.initSpeed * Math.cos(config.direction * (Math.PI / 180)));
  const [mouseY, setMouseY] = useState(-config.initSpeed * Math.cos(config.direction * (Math.PI / 180)));
  ...
  useEffect(() => {
    const loop = setInterval(() => {
      const mX = Math.abs(mouseX - mouseX0) < 1 ? mouseX0 : (mouseX + mouseX0) / 2;
      const mY = Math.abs(mouseY - mouseY0) < 1 ? mouseY0 : (mouseY + mouseY0) / 2;
      const a = -(Math.min(Math.max(-mY, -config.size), config.size) / config.radius) * config.maxSpeed;
      const b = Math.min(Math.max(-mX, -config.size), config.size) / config.radius * config.maxSpeed;
      const l = Math.PI / 180;
      ...
      items.forEach((item, index) => {
        ...
        const per = 2 * config.depth / (2 * config.depth + rz2);
        newArray.push({
          x: rx2,
          y: ry2,
          z: rz2
